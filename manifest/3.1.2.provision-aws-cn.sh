...
sed -i 's/${AWS_SSH_KEY_NAME}/'"${AWS_SSH_KEY_NAME}"'/g' yaml/_install/2.2.cluster-template-aws-${AWS_VERSION}.yaml
sed -i 's/${KUBERNETES_VERSION}/'"${KUBERNETES_VERSION}"'/g' yaml/_install/2.2.cluster-template-aws-${AWS_VERSION}.yaml
sed -i 's/${CONTROL_PLANE_MACHINE_COUNT}/'"${CONTROL_PLANE_MACHINE_COUNT}"'/g' yaml/_install/2.2.cluster-template-aws-${AWS_VERSION}.yaml
sed -i 's/${WORKER_MACHINE_COUNT}/'"${WORKER_MACHINE_COUNT}"'/g' yaml/_install/2.2.cluster-template-aws-${AWS_VERSION}.yaml
